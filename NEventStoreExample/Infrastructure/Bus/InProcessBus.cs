namespace NEventStoreExample.Infrastructure.Bus
{
  ...
  public class InProcessBus : IBus
  {
    ...
    public void Publish<T>(T @event) where T : IDomainEvent
    {
      ...
      {
        if (dispatchStrategy == DispatchStrategy.Asynchronous)
        {
    ...
    private static MethodInfo _createActionMethod;
    ...
    private IDisposable RegisterAllHandlersByReflection(object eventHandler)
    {
      ...
      RegisterAllEventHandlersByReflection(eventHandler, subscriptionDisposer);
      RegisterAllCommandHandlersByReflection(eventHandler, subscriptionDisposer);
    ...
    {
      _createActionMethod = this.GetType().GetMethod("CreatePublishAction");
      _registerMethod = this.GetType().GetMethod("RegisterEventHandler");
    ...
    {
      _createActionMethod = this.GetType().GetMethod("CreateSendAction");
      _registerMethod = this.GetType().GetMethod("RegisterCommandHandler");
      var genericArgs = handleEventType.GetGenericArguments();
      foreach (var genericArg in genericArgs)
      {
        var action = this.CreateTheProperAction(genericArg, eventHandler);
    ...
    {
      _registerMethod.MakeGenericMethod(handlerType).Invoke(this, new[] { action, subscriptionDisposer });
    }

    private object CreateTheProperAction(Type handlerType, object handler)
    {
      return _createActionMethod.MakeGenericMethod(handlerType, handler.GetType()).Invoke(this, new[] { handler });
    }
